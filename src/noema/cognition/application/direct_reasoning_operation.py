...
class DirectReasoningOperation:
    ...
    ``CanonicalInputIngestor``, one ``ContextRequestAssembler``, and one
    ...
    itself, and performs no I/O of its own; any error raised by the
    authority, the ingestor, C3, D1, or the ``ReasoningEngine`` propagates
    ...
    def __init__(
        ...
        *,
        ...
        """Bind the authority, ingestor, assembler, and engine this operation delegates to."""
    ...
    async def execute(
        ...
        *,
        ...
        is already resolvable through the authority. It then observes the
        canonical cognitive state exactly once, transitively, through the
        bound ``ContextRequestAssembler``; selects no context policy, budget
        values, or strategy; and returns the exact ``ReasoningOutcome``
        produced by the bound ``ReasoningEngine``. Any error raised while
        registering the content, ingesting the task, assembling the
        ``ContextRequest``, assembling the DIRECT ``ReasoningRequest``, or
